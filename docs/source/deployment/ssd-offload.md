...
| `MOONCAKE_OFFLOAD_LOCAL_BUFFER_SIZE_BYTES` | `1342177280` (1280 MiB, about 1.25 GiB) | Client-side staging buffer size used by the load path |
...
| `MOONCAKE_SCANMETA_ITERATOR_KEYS_LIMIT` | `20000` | Maximum keys per batch when scanning on-disk metadata during startup recovery |
